...
def preprocess(text):
    ...
    tokens = [stemmer.stem(t) for t in tokens if t not in stop_words]  # Stemming + eliminar stopwords
    doc = nlp(' '.join(tokens))                        # Procesar con spaCy para lematización
    lemmatized = [token.lemma_ for token in doc]       # Obtener lemas
    return ' '.join(lemmatized)                        # Devolver texto final
...
# AQUI DEBES CREAR LA CARPETA DATA ANTES DE GUARDAR
